...
由于用户网络环境问题，无法正常访问[Hugging Face官网](https://huggingface.co/)，所有HF相关的操作（检索仓库、下载模型等），必须通过[镜像站](https://hf-mirror.com/)进行。镜像站的API格式与原站相同。

此外，[Civitai](https://civitai.com/)也无法直接访问，下载模型尽量通过HF镜像站下载。若需从Civitai下载超过**100MB**的文件，必须说明模型大小并请求用户批准。**严禁直接下载大模型文件！**